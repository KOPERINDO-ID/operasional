...
function prosesPembayaranMultiple(pembayaran_id, number) {
	...
	if ($pay.val() == "" || $bank.val() == "" || $tgl.val() == "") {
		...
		console.log('Validation failed - Pay:', $pay.val(), 'Bank:', $bank.val(), 'Date:', $tgl.val());
		return false;
	}
	...
	// CRITICAL: Jika pakai base64, buat FormData manual (JANGAN dari form)
	// Untuk avoid conflict dengan field foto_bukti kosong di form
	var formData;

	if (useBase64) {

		formData = new FormData(); // FormData kosong
		// FIXED: Deklarasikan variabel fileInput dan hasFile
		var storageKey = 'foto_pembayaran_' + pembayaran_id + '_' + number;
		var fotoData = getFotoDataForUpload(storageKey);
		if (fotoData) {
			// Jika fotoData adalah Promise (dari file input)
			if (fotoData instanceof Promise) {
				fotoData.then(function (data) {
					// Tambahkan ke FormData
					var blob = base64ToBlob(data.base64Image);
					formData.append('foto_bukti_1', blob, data.fileName);

				});
			} else {
				// Jika fotoData adalah object langsung (dari camera/global storage)
				var blob = base64ToBlob(fotoData.base64Image);
				formData.append('foto_bukti_1', blob, fotoData.fileName);
			}
		} else {
			// Tidak ada foto, lanjut tanpa foto
			app.dialog.alert('Isi Data Pembayaran Dengan Lengkap (Foto Bukti Belum Ada)');
		}
		// Append foto FIRST
		formData.append('foto_bukti', base64FromStorage);
		formData.append('foto_bukti_type', 'base64'); // Flag untuk backend

		// Append fields lainnya manual
		...
		console.log('✅ Manual FormData prepared:');
		console.log('   foto_bukti length:', base64FromStorage.length);
		console.log('   foto_bukti_type:', 'base64');

	} else if (fileInput.files && fileInput.files.length > 0) {
		// Web: Pakai file input biasa (dari form)
		var fileToUpload = fileInput.files[0];
		console.log('💻 Using file input for upload (form FormData)');
		console.log('   Name:', fileToUpload.name);
		console.log('   Size:', fileToUpload.size, 'bytes');

		formData = new FormData($('#pembayaran_form_multiple_' + number + '_' + pembayaran_id)[0]);

		console.log('✅ Form FormData prepared:');
		console.log('   foto_bukti:', fileToUpload.name);

		// Note: FormData dari form sudah include semua fields
		// Tidak perlu append lagi

	} else {
		console.error('❌ No file or base64 to upload');
		app.dialog.alert('Error: File tidak ditemukan');
		return false;
	}

	// DEBUG: Log semua FormData sebelum kirim
	console.log('🚀 Final FormData about to send:');
	for (var pair of formData.entries()) {
		if (pair[0] === 'foto_bukti') {
			// Jangan log full base64, terlalu panjang
			console.log('   ' + pair[0] + ':', typeof pair[1],
				(typeof pair[1] === 'string' ? pair[1].substring(0, 50) + '... (length: ' + pair[1].length + ')' : pair[1]));
		} else {
	...
	jQuery.ajax({
		...
		success: function (data) {
			// Cleanup localStorage setelah upload success (seperti openCameraTerima)
			if (localStorage.getItem(storageKey)) {
				localStorage.removeItem(storageKey);
				console.log('✅ Cleaned up localStorage:', storageKey);
			}

			// Clear data-has-photo attribute
			if (fileInput) {
				fileInput.removeAttribute('data-has-photo');
				fileInput.removeAttribute('data-storage-key');
			}
    ...
    <form id="pembayaran_form_multiple_${idx}_${id}">
      <tr>
        ...
        <td style="border:1px solid white;" class="numeric-cell text-align-center">
          ...
          <!-- ========================================== -->
          <!-- ✅ FIX 1: Parameter onclick yang BENAR     -->
          <!-- ========================================== -->
          <label class="text-add-colour-black-soft bg-dark-gray-young button-small col button text-bold" 
          ...
          <!-- ========================================== -->
          <!-- ✅ FIX 2: Tambah INPUT FILE (PENTING!)     -->
          <!-- ========================================== -->
          <input type="file" 
...
// ============================================================
// REVISI: FOTO BUTTON WITH ACTION SHEET
// ============================================================
// Direvisi untuk sesuai dengan HTML template tplInputRow()
// yang menggunakan parameter: foto_urutan, pembayaran_id, idx
// ============================================================

/**
 * CATATAN PENTING:
 * - HTML template menggunakan: onclick="showUploadFotoOptions('foto_bukti_${idx}_${id}', ${id}, ${idx});"
 * - Parameter 1: foto_element_id (misalnya: 'foto_bukti_1_123')
 * - Parameter 2: pembayaran_id (ID pembayaran operasional)
 * - Parameter 3: idx (index/nomor urut pembayaran)
 */

/**
 * Fungsi utama yang dipanggil saat klik button FOTO
 * Menampilkan pilihan Camera atau File
 * 
 ...
 */
function showUploadFotoOptions(foto_element_id, pembayaran_id, idx) {
	...
	// Simpan data ke global variable untuk digunakan nanti
	window.currentFotoUploadData = {
		foto_element_id: foto_element_id,
		pembayaran_id: pembayaran_id,
		idx: idx,
		input_element: document.getElementById(foto_element_id),
		preview_element: document.getElementById(foto_element_id + '_view')
	};

	// Validasi element ada
	if (!window.currentFotoUploadData.input_element) {
		console.error('❌ Input element not found:', foto_element_id);
		alert('Error: Input element tidak ditemukan');
		return;
	}

	if (!window.currentFotoUploadData.preview_element) {
		console.error('❌ Preview element not found:', foto_element_id + '_view');
		alert('Error: Preview element tidak ditemukan');
		return;
	}

	// Cek apakah sudah ada foto yang diupload
	var currentPreview = window.currentFotoUploadData.preview_element.src;
	var hasExistingPhoto = currentPreview && currentPreview !== '' && currentPreview !== window.location.href;

	if (hasExistingPhoto) {
		// Jika sudah ada foto, tampilkan opsi lihat/ganti/hapus
		showExistingFotoOptionsNew(foto_element_id, pembayaran_id, idx);
	} else {
		// Jika belum ada foto, langsung tampilkan pilihan Camera/File
		showUploadChoices(foto_element_id, pembayaran_id, idx);
	}
}

/**
 * Menampilkan pilihan untuk foto yang sudah ada
 * Opsi: Lihat, Ganti, Hapus, Batal
 */
function showExistingFotoOptionsNew(foto_element_id, pembayaran_id, idx) {
	console.log('🖼️ Showing existing foto options');

	// Deteksi environment
	var isCordova = typeof navigator.camera !== 'undefined' && typeof Camera !== 'undefined';

	if (isCordova && typeof app !== 'undefined' && app.actions) {
		// Mobile: Gunakan Framework7 action sheet
		var buttons = [
			{
				text: '<i class="f7-icons" style="font-size:24px;margin-right:10px;color:#2196F3;">eye_fill</i> Lihat Foto',
				onClick: function () {
					console.log('👁️ View foto');
					viewExistingFotoNew(foto_element_id);
				}
			},
			{
				text: '<i class="f7-icons" style="font-size:24px;margin-right:10px;color:#FF9800;">arrow_2_circlepath_circle</i> Ganti Foto',
				onClick: function () {
					console.log('🔄 Replace foto');
					showUploadChoices(foto_element_id, pembayaran_id, idx);
				}
			},
			{
				text: '<i class="f7-icons" style="font-size:24px;margin-right:10px;color:#f44336;">trash_fill</i> Hapus Foto',
				onClick: function () {
					console.log('🗑️ Delete foto');
					clearPhotoPreview(foto_element_id);
				}
			},
			{
				text: 'Batal',
		...
		// Web: Gunakan native confirm
		var choice = confirm('Foto sudah ada.\n\nKlik OK untuk GANTI foto\nKlik Cancel untuk LIHAT foto');
 ...
 */
function viewExistingFotoNew(foto_element_id) {
	console.log('👁️ Viewing existing foto');

	var previewElement = document.getElementById(foto_element_id + '_view');
	if (!previewElement) {
		alert('Preview element tidak ditemukan');
		return;
	}

	var fotoSrc = previewElement.src;

	// Gunakan zoom_view jika tersedia
	if (typeof zoom_view === 'function') {
		zoom_view(fotoSrc);
	} else if (typeof app !== 'undefined' && app.photoBrowser) {
 ...
 * Clear photo preview
 */
function clearPhotoPreview(foto_element_id) {
	console.log('🗑️ Clearing photo preview');

	var inputElement = document.getElementById(foto_element_id);
	var previewElement = document.getElementById(foto_element_id + '_view');
	...
	// Clear dari global storage jika ada
	if (window.currentFotoUploadData &&
		window.currentFotoUploadData.foto_element_id === foto_element_id) {
		window.currentFotoUploadData.base64Image = null;
	}

	showToast('Preview foto dihapus');
}

/**
 * Menampilkan pilihan Camera atau File untuk upload
 */
function showUploadChoices(foto_element_id, pembayaran_id, idx) {
	console.log('📤 Showing upload choices');
	console.log('   Foto Element ID:', foto_element_id);
	console.log('   Pembayaran ID:', pembayaran_id);
	console.log('   Index:', idx);
	...
	if (isCordova && typeof app !== 'undefined' && app.actions) {
		...
		// MOBILE: Gunakan Framework7 action sheet
		// ========================================
		console.log('📱 Using mobile action sheet');

		var buttons = [
			{
				text: '<i class="f7-icons" style="font-size:24px;margin-right:10px;color:#4CAF50;">camera_fill</i> Ambil Foto',
				onClick: function () {
					console.log('📷 Camera selected');
					openCameraForFotoNew(foto_element_id, pembayaran_id, idx, Camera.PictureSourceType.CAMERA);
				}
			},
			{
				text: '<i class="f7-icons" style="font-size:24px;margin-right:10px;color:#2196F3;">photo_fill</i> Pilih dari Gallery',
				onClick: function () {
					console.log('🖼️ Gallery selected');
					openCameraForFotoNew(foto_element_id, pembayaran_id, idx, Camera.PictureSourceType.PHOTOLIBRARY);
				}
			},
			{
				text: 'Batal',
		...
		inputElement.onchange = function (e) {
			handleFotoFileSelectionNew(e, foto_element_id, pembayaran_id, idx);
		};
...
/**
 * Open camera untuk ambil foto (Cordova)
 */
function openCameraForFotoNew(foto_element_id, pembayaran_id, idx, sourceType) {
	console.log('📷 Opening camera...');
	console.log('   Source Type:', sourceType === Camera.PictureSourceType.CAMERA ? 'Camera' : 'Gallery');
	...
	navigator.camera.getPicture(
		function cameraSuccess(imageUri) {
			...
			getFileContentAsBase64Terima(imageUri, function (base64Image) {
				...
				// Simpan ke global storage
				if (!window.currentFotoUploadData) {
					window.currentFotoUploadData = {};
				}
				window.currentFotoUploadData.base64Image = base64Image;
				window.currentFotoUploadData.foto_element_id = foto_element_id;
				window.currentFotoUploadData.pembayaran_id = pembayaran_id;
				window.currentFotoUploadData.idx = idx;

				// Show preview
				showPhotoPreview(foto_element_id, base64Image);

				// Toast
				showToast('Foto berhasil diambil dari ' + (sourceType === Camera.PictureSourceType.CAMERA ? 'kamera' : 'gallery'));
			});
		},
		function cameraError(error) {
			console.error('❌ Camera error:', error);

			// Jangan tampilkan error jika user cancel
...
/**
 * Handle file selection dari file input (Web)
 */
function handleFotoFileSelectionNew(event, foto_element_id, pembayaran_id, idx) {
	console.log('📁 handleFotoFileSelectionNew called');
	...
	if (!file.type.match('image.*')) {
		alert('Error: File yang dipilih bukan gambar. Silakan pilih file gambar (JPG, PNG, dll)');
		event.target.value = '';
		return;
	}
	...
	if (file.size > maxSize) {
		alert('Error: Ukuran file terlalu besar. Maksimal 10MB');
		event.target.value = '';
		return;
	}

	if (file.size === 0) {
		alert('Error: File kosong (0 bytes)');
		event.target.value = '';
		return;
	}
	...
	reader.onload = function (e) {
		...
		// Toast
		showToast('File berhasil dipilih: ' + file.name);
	};
 ...
 * Show photo preview
 */
function showPhotoPreview(foto_element_id, base64Image) {
	console.log('🖼️ Showing photo preview');
 ...
 * 
 * @param {string} foto_element_id - ID element input file
 * @returns {object|null} Object berisi base64Image dan fileName, atau null jika tidak ada
 */
function getFotoDataForUpload(foto_element_id) {
	console.log('📤 getFotoDataForUpload called for:', foto_element_id);

	// Cek di global storage
	if (window.currentFotoUploadData &&
		window.currentFotoUploadData.foto_element_id === foto_element_id &&
		window.currentFotoUploadData.base64Image) {

		console.log('✅ Found foto data in global storage');

		return {
			base64Image: window.currentFotoUploadData.base64Image,
			fileName: 'foto_' + Date.now() + '.jpg',
			pembayaran_id: window.currentFotoUploadData.pembayaran_id,
			idx: window.currentFotoUploadData.idx
		};
	}

	// Cek di input element (untuk web)
	var inputElement = document.getElementById(foto_element_id);

	if (inputElement && inputElement.files && inputElement.files[0]) {
		console.log('✅ Found file in input element');

		// Return promise karena perlu convert to base64
	...
	console.log('⚠️ No foto data found');
	return null;
}
...
// ============================================================
// Untuk support code lama yang mungkin masih memanggil uploadFotoPembayaran
// dengan parameter berbeda

/**
 * Upload foto pembayaran (compatibility function)
 * Ini untuk backward compatibility jika ada code lama yang masih pakai
 */
function uploadFotoPembayaran(foto_urutan, pembayaran_id, valid_spy, keterangan_valid, isi_foto) {
	console.log('📸 uploadFotoPembayaran (legacy) called');
	console.log('   Redirecting to showUploadFotoOptions...');

	// Extract idx dari foto_urutan jika ada
	var idx = 1; // default
	if (foto_urutan && typeof foto_urutan === 'string') {
		var match = foto_urutan.match(/(\d+)/);
		if (match) {
			idx = parseInt(match[1]);
		}
	}

	// Construct foto_element_id
	var foto_element_id = 'foto_bukti_' + idx + '_' + pembayaran_id;

	// Call new function
	showUploadFotoOptions(foto_element_id, pembayaran_id, idx);
}

// ============================================================
// INISIALISASI
// ============================================================
console.log('✅ Foto Upload Functions Loaded (Revised for tplInputRow)');

// Global variable untuk menyimpan data upload saat ini
window.currentFotoUploadData = window.currentFotoUploadData || {};

// ============================================================
// CATATAN PENGGUNAAN:
// ============================================================
/**
 * Cara menggunakan foto data saat simpan pembayaran:
 * 
 * Di fungsi simpanPembayaranMultiple() atau sejenisnya:
 * 
 * var fotoData = getFotoDataForUpload('foto_bukti_1_123');
 * 
 * if (fotoData) {
 *     // Jika fotoData adalah Promise (dari file input)
 *     if (fotoData instanceof Promise) {
